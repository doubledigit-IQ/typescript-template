...
export async function main(ns: NS): Promise<void> {
    ...
    function deployAttackFile(oServers: UTILS.ServerData, sAttackFile: string) {
        for (let i = 0; i < Object.keys(oServers).length; i++) {
            ...
            if (serverExists(sServerToDeploy)) {
    ...
    function deployToServers(oServers: UTILS.ServerData, nHomeRamBuffer: number, nDefaultPartitionSize: number) {
        ...
        //     if (serverExists(sServerToDeploy)) {
        //         if (oServers[sServerToDeploy].bDeployable) {
        //             var nRamAvailable = oServers[sServerToDeploy].nRamAvailable;
        //             var nScriptRam = getRam("attack.js", sServerToDeploy);
        //             if (nScriptRam === 0 || nRamAvailable < nScriptRam) {
        //                 continue;
        //             }
        //             var nPartitionThreads = Math.floor(nRamAvailable / nScriptRam);
        //             if (nPartitionThreads > 0) {
        //                 const bHack = false;
        //                 const bGrow = false;
        //                 const bWeaken = false;
        //                 const bShare = true;
    ...
    while (bLoop) {
        ...
        // nHomeRamBuffer += getRam("contracts.js", "home");
